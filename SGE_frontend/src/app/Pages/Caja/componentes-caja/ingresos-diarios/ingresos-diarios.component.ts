...
import {
  CuadreDiario,
  IngresosDiariosArray,
} from 'src/app/Types/Caja/CuadreDiario';
...
export class IngresosDiariosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (storedDate != null) {
      ...
      this.ingresosDiarios.fechaIngreso = fechaActual
        .toISOString()
        .split('T')[0];
    }
  ...
  ModalEditar(ingreso: IngresosDiariosArray) {
    ...
    const fechaFormateada =
      typeof ingreso.fechaIngreso === 'string'
    ...
    this.ingresosDiarios = {
      ...
      fechaIngreso:
        fechaFormateada instanceof Date && !isNaN(fechaFormateada.getTime())
  ...
  getIngresosDiarios() {
    ...
    this.cajaService
      .getIngresosByFecha(this.fechaIngreso)
      .pipe(
        tap((data: any) => {
  ...
  createIngresosDiarios() {
    ...
    if (
      this.ingresosDiarios.fechaIngreso == null ||
      this.ingresosDiarios.fechaIngreso.trim() == ''
    ) {
      Swal.fire({
        icon: 'error',
    ...
    if (
      this.ingresosDiarios.tipoIngreso == null ||
      this.ingresosDiarios.tipoIngreso.trim() == ''
    ) {
      Swal.fire({
        icon: 'error',
    ...
    this.cajaService
      .getCuadreDiario(fechaInicial, fechaFinal)
      .pipe(
  ...
  crearIngresoDiario() {
    ...
    const [year, month, day] = this.ingresosDiarios.fechaIngreso
      .split('-')
      .map(Number);
    const fechaFormateada = new Date(year, month - 1, day);

    const fechaIngresoFormateadaEnviar = new Date(
      this.ingresosDiarios.fechaIngreso
    ).toISOString();
  ...
  guardarFecha() {
    ...
    console.log(
      'Fecha guardada en localStorage:',
      this.ingresosDiarios.fechaIngreso
    );
  }
  ...
  updateIngresosDiarios(ingreso: IngresosDiariosArray) {
    ...
    this.cajaService
      .updateIngresosDiarios(ingreso, this.idIngreso)
      .pipe(
        tap((data: any) => {
          Swal.fire({
            icon: 'success',
          ...
          var pos = this.ingresosDiariosArray.findIndex(
            (i: any) => i.idIngresosDiarios == this.idIngreso
          );
  ...
  deleteIngresosDiarios(idIngreso: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.cajaService
          .deleteIngresosDiarios(idIngreso)
          .pipe(
            tap(() => {
              Swal.fire({
              ...
              this.ingresosDiariosArray = this.ingresosDiariosArray.filter(
                (ingreso: any) => ingreso.idIngresosDiarios !== idIngreso
              );
            }),
            catchError((error: Error) => {
              Swal.fire(
                'Error al eliminar el ingreso diario',
                'No se pudo eliminar el ingreso diario correctamente en el sistema.',
                'error'
              );
              console.error(error);
              return of([]);
