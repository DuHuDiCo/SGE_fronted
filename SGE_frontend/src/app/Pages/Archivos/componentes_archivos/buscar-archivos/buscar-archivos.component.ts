...
export class BuscarArchivosComponent implements OnInit {
      ...
      (data: any) => {
        if (data) {
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Esta obligación no contiene archivos, puede subirlos a continuación',
            timer: 2500,
            iconColor: '#960010',
            confirmButtonColor: '#960010',
            customClass: {
  ...
  llenarCards(position: number, obligacion: string) {
    ...
    this.archivos.forEach((element: any, index: number) => {
      var tipo = this.tiposArchivos.find(
        (t: any) => t.tipoArchivo == element.tipoArchivo.tipoArchivo
      );
